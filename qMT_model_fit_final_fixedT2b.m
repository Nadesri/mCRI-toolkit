% FUNCTION [fv rnrm] = qMT_model_fit_final_fixedT2b( data_MT, MT_Off_Freq, alpha_MT, alpha_0, t_r, t_m, t_s, pulse_type, line_shape, table_update, B0, B1, B1_MT, mode, in16 (data_SPGR or PD), in17 (alpha_SPGR or R1), TR_SPGR, t_m_SPGR, T2b)
%
...
function [fv rnrm] = qMT_model_fit_final_fixedT2b( data_MT, MT_Off_Freq, alpha_MT, alpha_0, t_r, t_m, t_s, pulse_type, line_shape, table_update, B0, B1, B1_MT,  mode, in15, in16, TR_SPGR, t_m_SPGR, T2b)
...
if mode == 2 || mode == 3
    g_F = Lorentian_fast([],[],1,table_update,t_m(1));
end
...
% Wrapper for qMT_residuals
    function residual = qMT_SPGR_Signal_Wrapper(x)
        ...
        elseif mode == 2
            g_F = Lorentian_fast(vox_MT_Off_Freq,x(6,1));
            W_F = W .*g_F;
        end
        
        % Calculate the Line Shape for Bound Pool
        if ~exist('line_shape','var') || isempty(line_shape) ||  strcmp(line_shape, 'SuperLorentzian')
            g_B = superLorentian_fastPM(vox_MT_Off_Freq,T2b);
            W_B = W .*g_B;
            
        elseif strcmp(line_shape, 'Gaussian')
            E = pi * (w_1rms).^2 .*exp(-(2*pi*T2b*(vox_MT_Off_Freq)).^2/2);
            W_B = T2b./(sqrt(2*pi)).*E;
            
        else
            error('Unknown line shape. Available choices: SuperLorentzian (Neural Tissues), Gaussian (Agar)');
        end
        ...
        for ll = 1:numel(vox_alpha_SPGR)
            ...
            %             S_SPGR(ll) = qMT_Signal_final(x(3,1), k, vox_alpha_SPGR(ll), 0, 0, TR_SPGR(ll), x(1,1), x(2,1), 0, 0);
        end
    ...
    function residual = qMT_Signal_Wrapper(x)
        ...
        elseif mode == 3
            g_F = Lorentian_fast(vox_MT_Off_Freq,x(4,1));
            W_F = W .*g_F;
        end
        % Calculate the Line Shape for Bound Pool
        if ~exist('line_shape','var') || isempty(line_shape) ||  strcmp(line_shape, 'SuperLorentzian')
...
% Initial Guess for 3.0T Magnet
    function ig = InitialGuessHuman()
        ...
        ig(1,:) = [0.005    0         100000  ];
        % R1
        ig(2,:) = [1.000    0.2       10.00   ];
        % f 
        ig(3,:) = [0.100    0.001     0.500   ];
        % k 
        ig(4,:) = [10.00    0.0100    100.00  ];
        % T2_B
        ig(5,:) = [T2b      1.1e-6    49.9e-6 ];
        % T2_F
