...
function [fv rnrm] = qMT_model_fit_final_mtdiff( data_MTdiff, MT_Off_Freq, alpha_MT, alpha_0, t_r, t_m, t_s, pulse_type, line_shape, table_update, B0, B1, B1_MT,  mode, in15, in16, TR_SPGR, t_m_SPGR)
...
elseif mode == 0 || mode == 2
    data_SPGR   = in15;
    alpha_SPGR  = in16;
    max_val     = max(max(data_MTdiff(:)),max(data_SPGR(:)));
    data_MTdiff     = data_MTdiff/max_val;
...
% Wrapper for qMT_residuals
    function residual = qMT_SPGR_Signal_Wrapper(x)
        ...
        S_MT = qMT_Signal_final(x(3,1), x(4,1), vox_alpha_0', t_m', t_s, t_r', x(1,1), x(2,1), W_B*0,W_F*0)-...
            qMT_Signal_final(x(3,1), x(4,1), vox_alpha_0', t_m', t_s, t_r', x(1,1), x(2,1), W_B, W_F);
    ...
    function residual = qMT_Signal_Wrapper(x)
        ...
        S_MT = qMT_Signal_final(x(1,1), x(2,1), vox_alpha_0', t_m', t_s, t_r', vox_PD, vox_R1, W_B*0, W_F*0)-...
            qMT_Signal_final(x(1,1), x(2,1), vox_alpha_0', t_m', t_s, t_r', vox_PD, vox_R1, W_B, W_F);
...
% Initial Guess for 3.0T Magnet
    function ig = InitialGuessHuman()
        ...
        % f 
        ig(3,:) = [0.100    0.015     0.500   ];
        % k 
        ig(4,:) = [2.000    0.0100    100.00  ];
        % T2_B
